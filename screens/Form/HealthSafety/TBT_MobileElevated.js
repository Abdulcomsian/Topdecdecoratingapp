...
const TBTMOBILE = () =>{
    ...
    const [coshhArray,setCoshhArray]=useState([
        {mainTitle:"Mobile Elevated Work Platform (MEWP ",title:"Mobile Elevated Work Platforms (MEWP’s) are useful pieces of plant when used properly. However, they combine height with mobility and can be extremely dangerous if misused."},
        ...
        {title:"•	Consider refuelling options (LPG, Diesel etc). Refuelling should take place in the open air where practicable and the engine must be switched off."},
        {title:"•	Any diesel spillages etc should be cleaned up immediately."},
        {title:"•	MEWP’s must be subject to thorough examinations at least once every six months and should be subject to regular local inspections (weekly?), the findings of which should be recorded."},
    ])
    return(
